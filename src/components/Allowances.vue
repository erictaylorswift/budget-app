<template>
  <div>
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="allowance in allowances" :key="allowance.id">
          <td
            v-if="
              allowance.date >= budgets[0].start &&
                allowance.date <= budgets[0].end
            "
          >
...
export default {
  ...
  updated() {
    ...
    for (var i = 0; i < allowances.length; i++) {
      if (
        allowances[i].date >= budgetStart &&
        allowances[i].date <= budgetEnd
      ) {
